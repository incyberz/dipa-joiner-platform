...
if ($id_room and $status_room === null and $id_role == 2 and $param != 'aktivasi_room') {
  # ============================================================
  # WAJIB AKTIVASI SETELAH MEMBUAT ROOM DI TAHAP AWAL
  # ============================================================
  jsurl('?aktivasi_room');
} elseif ($param != 'manage_ta' and (!$ta_awal || !$ta_akhir)) {
  # ============================================================
  # WAJIB MANAGE TA JIKA INVALID 
  # ============================================================
  if ($id_role == 2) echo div_alert('danger', "TA $ta_aktif invalid setting. <a href=?manage_ta>Manage TA</a>");
  // jsurl('?manage_ta');
} elseif (!$sesi_aktif and $param != 'manage_sesi_aktif' and $id_room) {
...
if (!file_exists($konten)) {
  $konten_admin = "pages/admin/$param.php";
