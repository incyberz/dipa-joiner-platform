...
if(isset($_POST['btn_update_jadwal_kuliah'])){
  ...
  $jadwal_kuliah = "$_POST[tanggal_sesi] $_POST[jam_sesi]";
  ...
  if(mysqli_num_rows($q)){
    $s = "UPDATE tb_sesi_kelas SET jadwal_kuliah='$jadwal_kuliah' WHERE $id_sesi_and_kelas";
  }else{
    $s = "INSERT INTO tb_sesi_kelas (id_sesi,kelas,jadwal_kuliah) VALUES ($arr[0],'$arr[1]','$jadwal_kuliah')";
  }
  $q = mysqli_query($cn,$s) or die(mysqli_error($cn));
  ...
  if(isset($_POST['update_next_week'])){
    ...
    while($d2=mysqli_fetch_assoc($q)){
      ...
      $durasi = $i * 7;
      $new_jadwal_kuliah = date('Y-m-d H:i',strtotime("+$durasi day",strtotime($jadwal_kuliah)));


      if(mysqli_num_rows($q2)){
        $s2 = "UPDATE tb_sesi_kelas SET jadwal_kuliah='$new_jadwal_kuliah' WHERE id_sesi=$d2[id_sesi] AND kelas='$ckelas'";
      }else{
        $s2 = "INSERT INTO tb_sesi_kelas (id_sesi,kelas,jadwal_kuliah) VALUES ($d2[id_sesi],'$ckelas','$new_jadwal_kuliah')";
      }
