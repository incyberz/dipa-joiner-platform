...
if($id_bukti){
  ...
  $menit = round((strtotime($tanggal_upload)-strtotime($tanggal_mulai))/60,0);
  $jam = intval($menit/60);
  $sisa_menit = $menit % 60;
...
if($id_bukti){
  ...
  if($jenis=='latihan'){
    ...
    if($d['count_sublevel']){
      ...
      $form_bukti = "<div class=wadah>Sublevels: $d[count_sublevel]</div>";
    }else{
...
$tanggal_jenis_show = date('M d, H:i',strtotime($d['tanggal']));
$keterangan = "$d[ket]";
$basic_point_show = number_format($d['basic_point'],0);
$ontime_point_show = number_format($d['ontime_point'],0);
$ontime_dalam_show = eta(strtotime($tanggal_mulai) - strtotime('now') + $d['ontime_dalam']*60);
$ontime_deadline_show = eta(strtotime($tanggal_mulai) - strtotime('now') + $d['ontime_deadline']*60);

$tr_challenge_info = $jenis!='challenge' ? '' : "
  <tr>
    <td class='tebal abu'>Link bukti harus terdapat kata:</td>
    <td class='darkblue'>$d[link_includes]</td>
  </tr>
  <tr>
    <td class='tebal abu'>Link bukti tidak boleh ada kata:</td>
    <td class='darkblue'>$d[link_excludes]</td>
  </tr>
";
...
<div class=debug>
  ...
  <div>tanggal_".$jenis."2:<span id=tanggal_$jenis"."2__$id_assign_jenis>$d[tanggal]</span></div>
